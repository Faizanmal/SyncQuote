...
export class CrmOutboundSyncService {
  ...
  async syncProposalToCrm(proposalId: string): Promise<OutboundSyncResult[]> {
    ...
    for (const link of dealLinks) {
      ...
      try {
        const result = await this.syncToProvider(
          link.crmIntegration,
          link.externalDealId,
          proposalId,
        );
  ...
  private async uploadHubspotAttachment(
    accessToken: string,
    dealId: string,
    proposal: any,
  ): Promise<void> {
  ...
  private async syncToPipedrive(integration: any, dealId: string, proposal: any): Promise<void> {
    ...
    if (Object.keys(updateData).length > 0) {
      await axios.put(`https://api.pipedrive.com/v1/deals/${dealId}`, updateData, {
  ...
  async triggerSync(
    proposalId: string,
    event: 'signed' | 'approved' | 'sent' | 'viewed',
  ): Promise<void> {
