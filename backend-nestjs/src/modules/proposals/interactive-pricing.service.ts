...
export class InteractivePricingService {
  ...
  async calculatePricing(
    ...
    const selectionsMap = new Map(dto.selections.map((s) => [s.itemId, s]));
    ...
    if (proposal.depositRequired && depositAmount && this.stripe) {
      try {
        const paymentIntentClientSecret = await this.updatePaymentIntent(proposal, depositAmount);
   ...
   */
  private async updatePaymentIntent(proposal: any, depositAmount: number): Promise<string | null> {
